...
export default function Register() {
  const [form, setForm] = useState({
    ...
    universite: '', autreUniversite: '',
    filiere: '', autreFiliere: '', niveau: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    const universiteFinale = form.universite === 'Autre' ? form.autreUniversite : form.universite;
    const filiereFinale = form.filiere === 'Autre' ? form.autreFiliere : form.filiere;
  ...
  const s = {
    ...
    card: { background: 'white', borderRadius: 16, padding: '2.5rem', width: '100%', maxWidth: 480, boxShadow: '0 2px 20px rgba(0,0,0,0.06)' },
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, textAlign: 'center', marginBottom: 4 }}>Créer un compte</h1>
        <p style={{ fontSize: 14, color: '#888', textAlign: 'center', marginBottom: '1.5rem' }}>Rejoins StudyMate AI</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input style={s.input} type="text" placeholder="Ton nom complet"
          ...
          <select style={s.select} value={form.universite} onChange={e => set('universite', e.target.value)}>
            <option value="">Sélectionne ton établissement...</option>
            {ETABLISSEMENTS.map(e => <option key={e} value={e}>{e}</option>)}
          </select>
          {form.universite === 'Autre' && (
            <input style={{ ...s.input, marginTop: -8 }} type="text"
              placeholder="Nom de ton établissement"
              value={form.autreUniversite} onChange={e => set('autreUniversite', e.target.value)} />
          )}
          ...
          <div style={s.row}>
            <div>
              ...
              <select style={s.select} value={form.filiere} onChange={e => set('filiere', e.target.value)}>
                ...
                {FILIERES_PRINCIPALES.map(f => <option key={f} value={f}>{f}</option>)}
              </select>
              {form.filiere === 'Autre' && (
                <input style={{ ...s.input, marginTop: -8 }} type="text"
                  placeholder="Nom de ta filière"
                  value={form.autreFiliere} onChange={e => set('autreFiliere', e.target.value)} />
              )}
            </div>
            <div>
          ...
          <div style={s.pwdWrap}>
            <input style={{ ...s.pwdInput, borderColor: '#ddd' }}
          ...
          <div style={s.pwdWrap}>
            <input style={{ ...s.pwdInput, borderColor: form.confirm && form.confirm !== form.password ? '#D85A30' : '#ddd' }}
          ...
          {form.confirm && form.confirm !== form.password && (
            <p style={{ fontSize: 12, color: '#D85A30', marginTop: -10, marginBottom: 12 }}>
